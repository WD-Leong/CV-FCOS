...
def build_model(
    num_classes, img_dims, n_layers=3, n_heads=4, 
    d_model=256, d_ffwd=512, backbone_model="resnet50"):
    """
...
class FCOSAttn(tf.keras.Model):
    def __init__(
        self, num_classes, img_dims, box_scale, 
        id_2_label, n_layers=3, n_heads=4, d_model=256, 
        d_ffwd=512, backbone_model="resnet50", **kwargs):
        super(FCOSAttn, self).__init__(name="FCOSAttn", **kwargs)
        self.model = build_model(
            num_classes, img_dims, n_layers=n_layers, 
