...
class SelfAttnLayer(tf.keras.layers.Layer):
    ...
    def call(self, q, mask=None):
        ...
        for n_layer in range(self.n_layers):
            ...
            attn_mha = tmp_layer_norm_1(
                tf.add(mha_out[0], attn_in))
            
            # Point-wise feed forward network. #
            attn_ffw1 = tf.nn.relu(
                self.ffwd_layer_1[n_layer](attn_mha))
            attn_ffw2 = self.ffwd_layer_2[n_layer](attn_ffw1)
            
            attn_out = tmp_layer_norm_2(
                tf.add(attn_mha, attn_ffw2))
            attn_in  = attn_out
        return attn_out
...
def build_model(
    ...
    if backbone_model.lower() == "resnet50":
        ...
        c3_c5_layer_names = [
            "conv3_block3_out", 
            "conv4_block5_out", "conv5_block3_out"]
        ...
        c3_c5_layer_names = [
            "conv3_block3_out", 
            "conv4_block22_out", "conv5_block3_out"]
    ...
    for n_output in range(len(fpn_output)):
        ...
        cls_output = layers.Conv2D(
            ...
            bias_initializer=b_focal, 
            name="cls_output_"+str(n_output+1))(tmp_output)
