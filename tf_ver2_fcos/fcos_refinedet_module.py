...
def build_model(
    ...
    for n_layer in range(4):
        arm_cls_cnn.append(layers.Conv2D(
            256, 3, padding="same", 
            activation=None, use_bias=False, 
            name="arm_cls_layer_" + str(n_layer+1)))
        
        obm_cls_cnn.append(layers.Conv2D(
            256, 3, padding="same", 
            activation=None, use_bias=False, 
            name="obm_cls_layer_" + str(n_layer+1)))
        
        arm_reg_cnn.append(layers.Conv2D(
            256, 3, padding="same", 
            activation=None, use_bias=False, 
            name="arm_reg_layer_" + str(n_layer+1)))
        
        obm_reg_cnn.append(layers.Conv2D(
            256, 3, padding="same", 
            activation=None, use_bias=False, 
            name="obm_reg_layer_" + str(n_layer+1)))
    ...
    p6_output = layers.Conv2D(
        256, 3, 2, "same", name="c6_3x3")(p5_output)
    ...
    for n_output in range(len(fpn_output)):
        ...
        for n_layer in range(4):
            layer_cls_output = \
                arm_cls_cnn[n_layer](layer_cls_output)
        
        tmp_output = tf.nn.relu(layer_cls_output)
        cen_output = layers.Conv2D(
            1, 3, 1, padding="same", 
            bias_initializer=b_focal, 
            name="arm_cen_out_"+str(n_output+1))(tmp_output)
        cen_heads.append(cen_output)
    ...
    for n_output in range(len(fpn_output)):
        ...
        for n_layer in range(4):
            layer_reg_output = \
                arm_reg_cnn[n_layer](layer_reg_output)
        
        tmp_output = tf.nn.relu(layer_reg_output)
        reg_output = layers.Conv2D(
            4, 3, 1, padding="same", use_bias=True, 
            name="arm_reg_out_"+str(n_output+1))(tmp_output)
        reg_heads.append(reg_output)
    
    arm_out = []
    for n_level in range(len(fpn_output)):
        arm_out.append(tf.concat(
            [reg_heads[n_level], 
             cen_heads[n_level]], axis=3))
    
    # Transfer Connection Block. #
    rev_arm = arm_out[::-1]
    rev_shp = tf.shape(rev_arm[0])
    res_in  = tf.zeros([
        rev_shp[0], rev_shp[1], rev_shp[2], 256])
    
    obm_out = []
    for n_level in range(len(arm_out)):
        tcb_name_1 = "tcb_" + str(n_level+1) + "_cnn_1"
        tcb_name_2 = "tcb_" + str(n_level+1) + "_cnn_2"
        tcb_name_3 = "tcb_" + str(n_level+1) + "_out"
        
        tmp_tcb = layers.Conv2D(
            256, 1, 1, "same", 
            name=tcb_name_1)(rev_arm[n_level])
        tmp_tcb = layers.Conv2D(
            256, 3, 1, "same", name=tcb_name_2)(tmp_tcb)
        tmp_tcb = tf.nn.relu(tmp_tcb)
        
        if n_level == 0:
            tmp_out = tmp_tcb + res_in
        else:
            ups_name = "ups_" + str(n_level+1)
            tmp_out = tmp_tcb + layers.UpSampling2D(
                size=(2, 2), name=ups_name)(res_in)
    
        tmp_out = tf.nn.relu(tmp_out)
        tmp_out = layers.Conv2D(
            256, 3, 1, "same", name=tcb_name_3)(tmp_out)
        tmp_out = tf.nn.relu(tmp_out)
        
        layer_reg_output = tmp_out
        for n_layer in range(4):
            layer_reg_output = \
                obm_reg_cnn[n_layer](layer_reg_output)
        
        layer_cls_output = tmp_out
        for n_layer in range(4):
            layer_cls_output = \
                obm_cls_cnn[n_layer](layer_cls_output)
        
        reg_output = tf.nn.relu(layer_reg_output)
        reg_output = layers.Conv2D(
            4, 3, 1, padding="same", 
            name="reg_output_"+str(n_level+1))(reg_output)
        
        cls_output = tf.nn.relu(layer_cls_output)
        cls_output = layers.Conv2D(
            num_classes, 3, 1, padding="same",
            bias_initializer=b_focal, 
            name="cls_output_"+str(n_level+1))(cls_output)
        
        res_in = tmp_out
        obm_out.append(
            tf.concat([reg_output, cls_output], axis=3))
    obm_out = obm_out[::-1]
    
    x_outputs = []
    for n_level in range(len(arm_out)):
        x_outputs.append([
            arm_out[n_level], obm_out[n_level]])
    return tf.keras.Model(
        inputs=backbone.input, outputs=x_outputs)
...
class FCOS(tf.keras.Model):
    ...
    def train_loss(self, x_image, x_label):
        ...
        Note that the ground truth label has center output incorporated
        but the ARM and OBM modules are separate.
        """
        ...
        for n_scale in range(len(x_pred)):
            ...
            arm_pred = x_pred[n_scale][0]
            obm_pred = x_pred[n_scale][1]
            
            cls_loss += self.focal_loss(
                x_label[n_scale][..., 5:], obm_pred[0][..., 4:])
            
            cen_loss += self.focal_loss(
                x_label[n_scale][..., 4], arm_pred[0][..., 4])
            
            reg_loss_arm += self.smooth_l1_loss(
                x_label[n_scale][..., :4], 
                tf.nn.sigmoid(arm_pred[0][..., :4]), mask=tmp_mask)
            reg_loss_obm += self.smooth_l1_loss(
                x_label[n_scale][..., :4], 
                tf.nn.sigmoid(obm_pred[0][..., :4]), mask=tmp_mask)
        return cls_loss, reg_loss_obm, cen_loss, reg_loss_arm
    ...
    def image_detections(
        ...
        for n_layer in range(len(tmp_predict)):
            ...
            tmp_arm = tmp_predict[n_layer][0]
            tmp_obm = tmp_predict[n_layer][1]
            
            processed_center = \
                np.expand_dims(tf.nn.sigmoid(
                    tmp_arm[0][..., 4].numpy()), axis=2)
            processed_output = tmp_obm.numpy()[0]
            processed_logits = processed_output[..., 4:]
            raw_box_outputs  = processed_output[..., :4]
            processed_bboxes = self.prediction_to_corners(
                tf.nn.sigmoid(raw_box_outputs), tmp_sc, stride)
            
            processed_output[..., :4] = processed_bboxes
            processed_output[..., 4:] = \
                tf.nn.sigmoid(processed_logits).numpy()
            
            out_dim = processed_output.shape
            tmp_reg = processed_output[:, :, :4]
            tmp_cls = processed_output[:, :, 4:]
            tmp_arr = np.concatenate(tuple([
                tmp_reg, processed_center, tmp_cls]), axis=2)
            
            tmp_outputs.append(np.array([
                tmp_arr[x, y, :] for x in range(
                out_dim[0]) for y in range(out_dim[1])]))
