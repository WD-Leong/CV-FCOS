...
from tf_ver2_fcos import fcos_refinedet_module
    ...
    for step in range(st_step, max_steps):
        ...
        for tmp_idx in batch_sample:
            ...
            with tf.GradientTape() as grad_tape:
                ...
                tot_losses = tf.add(
                    tmp_losses[0] + tmp_losses[1], 
                    tmp_losses[2] + tmp_losses[3])
            
            # Accumulate the gradients. #
            num_object += sum(n_labels)
            cls_losses += tmp_losses[0]
            o_reg_loss += tmp_losses[1]
            cen_losses += tmp_losses[2]
            a_reg_loss += tmp_losses[3]
        ...
        tot_a_reg_loss += a_reg_loss / batch_size
        tot_o_reg_loss += o_reg_loss / batch_size
        ...
        if (step+1) % display_step == 0:
            ...
            avg_a_reg_loss = tot_a_reg_loss / display_step
            avg_o_reg_loss = tot_o_reg_loss / display_step
            
            avg_cls_loss = avg_cls_loss.numpy()
            avg_cen_loss = avg_cen_loss.numpy()
            avg_a_reg_loss = avg_a_reg_loss.numpy()
            avg_o_reg_loss = avg_o_reg_loss.numpy()
            
            training_loss.append((
                step+1, avg_loss.numpy(), 
                avg_cen_loss, avg_cls_loss, 
                avg_a_reg_loss, avg_o_reg_loss))
            ...
            print("Average ARM Reg Loss:", str(round(avg_a_reg_loss, 5)))
            print("Average OBM Reg Loss:", str(round(avg_o_reg_loss, 5)))
            ...
            if (step+1) % step_save == 0:
                ...
                train_df_cols = [
                    "step", "train_loss", "cen_loss", 
                    "cls_loss", "a_reg_loss", "o_reg_loss"]
                train_loss_df = pd.DataFrame(
                    training_loss, columns=train_df_cols)
...
train_loss = \
    model_path + "coco_losses_fcos_refinedet_resnet50.csv"
ckpt_model = model_path + "coco_fcos_refinedet_resnet50"
restore_flag = True
...
fcos_model = fcos_refinedet_module.FCOS(
    num_classes, box_scale, 
    label_2_id, backbone_model="resnet50")
